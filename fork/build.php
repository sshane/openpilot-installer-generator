...
define("NUM_USERNAME_CHARS", iconv_strlen(E));
define("NUM_LOADING_CHARS", iconv_strlen(PI));
...
$num_nulls_append = NUM_USERNAME_CHARS - iconv_strlen($username);  # number of spaces we need to append to end of string before NUL
$branch_start_idx = strpos($installer_binary, BRANCH_START_STR) + iconv_strlen(BRANCH_START_STR);
...
if ($branch != "") {
    ...
    $branch_start_idx = strpos($installer_binary, BRANCH_START_STR) + iconv_strlen(BRANCH_START_STR);
    $branch_len = iconv_strlen($branch) + 4;  # +4 for " -b "
